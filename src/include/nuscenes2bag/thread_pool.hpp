...
public:
  prioritized_task(int p, task_t&& f)
    : priority(p)
    , task(f)
  {
  }
...
public:
  ThreadPool(ThreadPool& to_copy) =
    delete; // Probably wouldn't and shouldn't copy this.
  ...
  ~ThreadPool(void) { this->close(); }
};

template<>
ThreadPool<FIFO>::ThreadPool(size_t numThreads)
  : isActive(true)
{
  for (size_t i = 0; i < numThreads; i++) {
