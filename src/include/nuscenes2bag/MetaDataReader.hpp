...
public:
  ...
  boost::optional<SceneInfo> getSceneInfo(
    const Token& sceneToken) const override;

  std::vector<SampleDataInfo> getSceneSampleData(
    const Token& sceneToken) const override;
  std::vector<EgoPoseInfo> getEgoPoseInfo(
    const Token& sceneToken) const override;
  CalibratedSensorInfo getCalibratedSensorInfo(
    const Token& calibratedSensorToken) const override;
  std::vector<CalibratedSensorInfoAndName> getSceneCalibratedSensorInfo(
    const Token& sceneToken) const override;
  CalibratedSensorName getSensorName(const Token& sensorToken) const override;

  boost::optional<SceneInfo> getSceneInfoByNumber(
    const uint32_t sceneNumber) const override;

private:
  static nlohmann::json slurpJsonFile(const fs::path& filePath);
  static std::vector<SceneInfo> loadScenesFromFile(const fs::path& filePath);
  static std::map<Token, std::vector<SampleInfo>> loadSampleInfos(
    const fs::path& filePath);
  static std::map<Token, std::vector<SampleDataInfo>> loadSampleDataInfos(
    const fs::path& filePath);
  static std::map<Token, std::vector<EgoPoseInfo>> loadEgoPoseInfos(
    const fs::path& filePath,
    std::map<Token, Token> sample2SampleData);
  static std::map<Token, CalibratedSensorInfo> loadCalibratedSensorInfo(
    const fs::path& filePath);
  static std::map<Token, CalibratedSensorName> loadCalibratedSensorNames(
    const fs::path& filePath);
  ...
  std::map<Token, std::set<CalibratedSensorInfoAndName>>
    scene2CalibratedSensorInfo;
